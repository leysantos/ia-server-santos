        ...
        emit_evolution_signal(
            {
                ...
                "input_text": text,
                "task_type": "orchestration_synthesis",
                "discipline": plan.disciplines[0] if plan.disciplines else "GERAL",
                "output_quality": evaluation.get("final_score") if isinstance(evaluation, dict) else None,
                "success": execution.error_count == 0,
                "extra": {"models_used": models_used, "evaluation": evaluation},
            }
        )
    except Exception:
        pass
