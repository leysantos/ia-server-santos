...
class DatabaseRepository:
    ...
    def serialize_project(project: Project, *, include_children: bool = False) -> dict[str, Any]:
        data: dict[str, Any] = {
            ...
            "codigo": getattr(project, "codigo", None),
            "cliente": getattr(project, "cliente", None),
            "responsavel": getattr(project, "responsavel", None),
            "disciplina": getattr(project, "disciplina", None),
            "status": getattr(project, "status", "ativo"),
            "versao_atual": getattr(project, "versao_atual", "REV00"),
            "workflow_initialized": getattr(project, "workflow_initialized", False),
            "empresa_id": str(project.empresa_id) if getattr(project, "empresa_id", None) else None,
